class Game {
  ...
  placeEnemies(count = 10) {
    while (this.enemies.length < count) {
      ...
      if (!this.enemies.some((e) => e.x === x && e.y === y) && this.map[y][x] === 'empty') {
  ...
  moveEnemies() {
    this.enemies.forEach((enemy) => {
      ...
      // Определяем приоритетную ось: где расстояние больше. true - если по горизонтали
        ...
        if (newX >= 0 && newX < this.tilesX && newY >= 0 && newY < this.tilesY) {
          ...
          if (targetTile === 'empty' || targetTile === 'tileHP') {
            ...
            this.renderer.updateTile(
              oldX,
              oldY,
              this.map[oldY][oldX] === 'tileHP' ? 'tileHP' : 'tile-empty',
            )
    ...
    dirs.forEach(([dx, dy]) => {
      ...
      if (enemy) {
        ...
        if (enemy.health <= 0) {
          ...
          this.renderer.updateTile(enemy.x, enemy.y, wasOnPotion ? 'tileHP' : 'tile-empty')
        } else {
